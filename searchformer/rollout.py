...
def rollout(
    ...
    worker = RolloutWorker(params=params, rank=rank, world_size=world_size, prefix=prefix)
    worker.rollout_test(
...
@click.option(
    ...
    type=str,
    default='',
    help="Doing slow mode or fast mode",
)
def probability(
    checkpoint_id: str,
    dataset_name: str,
    ...
    to constrain the dataset slice included this rollout dataset.
    """
    if (prefix not in ['create', 'plan', '']):
        logging.debug("error prefix choice")
        return
    logging.info(f"Rolling out checkpoint {checkpoint_id} with prefix {prefix}")
    params = RolloutParameter(
