...
class AStarTraceIterableDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[AStarTrace]:
        ...
        for batch in batch_loader:
            tensor_list = self.tokenizer.tokenize_batch(batch, self.plan_only, self.args_dict)
            if self.shuffle:
...
class TrainRun:
    ...
    def __init__(self, config: TrainConfig, args_dict: Dict):
        self.config = config
...
def _train(run_id: str, **args):
    ...
    no_close = float(args.get('no_close', 0.0))
    no_cost = float(args.get('no_cost', 0.0))
    no_create = float(args.get('no_create', 0.0))
    no_trace = float(args.get('no_trace', 0.0))
    env = args.get('env', 'maze')
    args_dict = copy.copy(args)
    logging.info(f'no_close = {no_close} | no_cost = {no_cost} | no_create = {no_create} | no_trace = {no_trace}')
    logging.debug(f'no_close = {no_close} | no_cost = {no_cost} | no_create = {no_create} | no_trace = {no_trace}')
    print(f'no_close = {no_close} | no_cost = {no_cost} | no_create = {no_create} | no_trace = {no_trace}')
    del args['no_close'] #= no_close
    del args['no_cost'] #= no_cost
    del args['no_create']# = no_create
    del args['no_trace'] #= no_trace
    del args['env']
    if 'experiment_id' in args: del args['experiment_id'] #= no_trace
    if 'description' in args: del args['description'] #= no_trace


    run_data = TrainRunData()
    run = TrainRun(TrainConfig.from_args(run_id=run_id, **args), args_dict)
    logging.info(f"Run id: {run.config.run_id}")
    if run.rank == 0:
        logging.info(f"Args: {args}")
        logging.info("{}".format(args_dict))
