...
class DictTokenizer:
    ...
    def tokenize_sokoban(
        self, trace: TokenizedTrace, plan_only: bool = False, args_dict: Dict = {},
    ) -> AStarTrace:
        ...
        def extract_sequences(data):
            ...
            for word in data:
                if word == 'create':
                    if temp:
                        ...
                        temp = []
                    capture = True
                elif word == 'close' or word == 'create':
                    if temp:
                        sequences.append(temp)
                        temp = []
                    capture = False
            ...
            if temp:
                sequences.append(temp)
            # return sequences
            flattened_list = list(itertools.chain(*sequences))
            return flattened_list

        def extract_sequences_exclude_c(data):
            sequences = []
            temp = []
            capture = False
            for word in data:
                if word == 'create':
                    ...
                    capture = False
                if capture:
                    if word[0] == 'c' and word != 'create':continue
                    temp.append(word)
            if temp:
                sequences.append(temp)
            # return sequences
        ...
        def extract_sequences_with_drop(data, drop_probability=0.3):
            sequences = []
            temp = []
            capture = False
            for word in data:
            ...
            if temp:
                sequences.append(temp)
            filtered_sequences = [seq for seq in sequences if random.random() > drop_probability]
            flattened_list = list(itertools.chain(*filtered_sequences))
            return flattened_list

        if trace.id not in self.visted:
            reason_list = extract_sequences(copy.copy(trace.reasoning))
            reason_list2 = extract_sequences_exclude_c(copy.copy(reason_list))
            reason_list3 = extract_sequences_with_drop(copy.copy(reason_list))

            # Example usage:
            #data_list = ['create', 'x1', 'y1', 'create', 'x2', 'y2', 'create', 'x3', 'y3', 'create', 'x4', 'y4']
            # reason_list3 = drop_random_triplets(copy.copy(reason_list2))
            #print("before = {}\n\n\nafter = {}".format(trace.reasoning, reason_list))
            # Generate a random float between 0 and 1
            random_number = random.random()
            no_close = args_dict['no_close']
            no_cost = args_dict['no_cost'] 
            no_create = args_dict['no_create']
            no_trace = args_dict['no_trace']
            complete_trace = 1.0 - no_close - no_cost - no_create - no_trace
            assert complete_trace >= -0.00001
            if random_number < no_close:
                trace.reasoning = reason_list
            elif random_number < no_close + no_cost:
                trace.reasoning = reason_list2
            elif random_number < no_close + no_cost + no_create:
                trace.reasoning = reason_list3
            elif random_number < no_close + no_cost + no_create + no_trace:
                trace.reasoning = []
            self.visted[trace.id] = trace.reasoning 
        else:
            trace.reasoning = self.visted[trace.id]
    ...
    def tokenize_maze(
        ...
        prompt_idx = self.encode(prompt_tok)
        no_close_reasoning = []
        no_close_no_cost_reasoning = []
        for i in range(len(trace.reasoning)):
            if trace.reasoning[i] == 'create':
                no_close_reasoning.append(trace.reasoning[i])
                no_close_reasoning.append(trace.reasoning[i+1])
                no_close_reasoning.append(trace.reasoning[i+2])
                no_close_reasoning.append(trace.reasoning[i+3])
                no_close_reasoning.append(trace.reasoning[i+4])

                no_close_no_cost_reasoning.append(trace.reasoning[i])
                no_close_no_cost_reasoning.append(trace.reasoning[i+1])
                no_close_no_cost_reasoning.append(trace.reasoning[i+2])
            else:
                continue

        import random
        def drop_random_triplets(data_list, min_percent=10, max_percent=40):
            num_triplets = len(data_list) // 5
            percent_to_drop = random.randint(min_percent, max_percent)
            num_to_drop = num_triplets * percent_to_drop // 100
            indices_to_drop = set(random.sample(range(num_triplets), num_to_drop))
            new_list = []
            for i in range(num_triplets):
                if i not in indices_to_drop:
                    new_list.extend(data_list[i*5:(i+1)*5])

            return new_list

        no_close_no_cost_no_create_reasoning = drop_random_triplets(copy.copy(no_close_reasoning))
        ...
        assert complete_trace >= -0.00001
        if random_number < no_close:
            trace.reasoning = no_close_reasoning
        elif random_number < no_close + no_cost:
            trace.reasoning = no_close_no_cost_reasoning
        elif random_number < no_close + no_cost + no_create:
            trace.reasoning = no_close_no_cost_no_create_reasoning
        elif random_number < no_close + no_cost + no_create + no_trace:
            trace.reasoning = []
    ...
    def tokenize(
        ...
        if args_dict.get('env', None) != 'sokoban':
            return self.tokenize_maze(trace, plan_only, args_dict)
        else:
            return self.tokenize_sokoban(trace, plan_only, args_dict)


    def tokenize_batch(
        self, trace_it: Iterable[TokenizedTrace], plan_only: bool = False, args_dict: Dict = {},
    ) -> List[AStarTrace]:
        tok_list: List[AStarTrace] = []
        for trace in trace_it:
            tok_list.append(self.tokenize(trace, plan_only=plan_only, args_dict=args_dict))
        return tok_list
